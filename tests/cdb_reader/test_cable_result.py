...
class SOFiSTiKCDBReaderCableResultTestSuite(TestCase):
    def setUp(self) -> None:
        ...
        self.cdb.open()
        self.cdb.cables.results.load(self.load_cases)
    ...
    def test_data(self) -> None:
        ...
        assert_frame_equal(data, self.cdb.cables.results.get_data(), rtol=1E-7)

    def test_get(self) -> None:
        with self.subTest(msg="Axial force"):
            self.assertEqual(
                self.cdb.cables.results.get(102, 1000, "AXIAL_FORCE"),
                1.7247849702835083
            )

        with self.subTest(msg="Average axial force"):
            self.assertEqual(
                self.cdb.cables.results.get(102, 1000, "AVG_AXIAL_FORCE"),
                1.7247587442398071
            )

        with self.subTest(msg="Axial displacement"):
            self.assertEqual(
                self.cdb.cables.results.get(101, 1001, "AXIAL_DISPLACEMENT"),
                8.366186521016061e-04
            )

        with self.subTest(msg="Relaxed length"):
            self.assertEqual(
                self.cdb.cables.results.get(103, 1001, "RELAXED_LENGTH"),
                1
            )

        with self.subTest(msg="Total strain"):
            self.assertEqual(
                self.cdb.cables.results.get(102, 1001, "TOTAL_STRAIN"),
                1.0000000031710769e-30
            )

        with self.subTest(msg="Effective stiffness"):
            self.assertEqual(
                self.cdb.cables.results.get(102, 1000, "EFFECTIVE_STIFFNESS"),
                0.8180915713310242
            )

        with self.subTest(msg="Non existing entry without default"):
            with self.assertRaises(LookupError):
                self.cdb.cables.results.get(102, 1000, "NON-EXISTING")

        with self.subTest(msg="Non existing entry with default"):
            self.assertEqual(
                self.cdb.cables.results.get(102, 1000, "NON-EXISTING", 5),
                5
            )

    def test_get_after_clear(self) -> None:
        self.cdb.cables.results.clear(1000)
        with self.subTest(msg="Check clear method"):
            with self.assertRaises(LookupError):
                self.cdb.cables.results.get(102, 1000, "AXIAL_FORCE")

        self.cdb.cables.results.load(1000)
        with self.subTest(msg="Check indexes management"):
            self.assertEqual(
                self.cdb.cables.results.get(103, 1000, "RELAXED_LENGTH"),
                1
            )

    def test_get_after_clear_all(self) -> None:
        self.cdb.cables.results.clear_all()
    ...
    def test_load_with_duplicated_load_cases(self) -> None:
        self.cdb.cables.results.clear_all()
        self.cdb.cables.results.load(self.load_cases + [1000])
        self.assertEqual(
            self.cdb.cables.results.get(103, 1000, "RELAXED_LENGTH"),
            1
        )

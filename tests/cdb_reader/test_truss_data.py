...
class SOFiSTiKCDBReaderTrussDataTestSuite(TestCase):
    ...
    def test_data(self) -> None:
        ...
        assert_frame_equal(self.expected_data, self.cdb.truss_data.data(), rtol=1E-7)

    def test_get(self) -> None:
        """Test for the `get` method.
        """
        with self.subTest(msg="First node id"):
            self.assertEqual(self.cdb.truss_data.get(2002, "N1"), 2)

        with self.subTest(msg="Second node id"):
            self.assertEqual(self.cdb.truss_data.get(1001, "N2"), 2)

        with self.subTest(msg="Initial length"):
            self.assertEqual(self.cdb.truss_data.get(1001, "L0"), 5.024937629699707)
